...
INSERT INTO `sys_template` (`pid`, `tstamp`, `sorting`, `crdate`, `cruser_id`, `title`, `root`, `clear`, `include_static_file`, `constants`, `config`) VALUES
	(1, UNIX_TIMESTAMP(), 256, UNIX_TIMESTAMP(), 2, 'NEW SITE', 1, 3, 'EXT:fluid_styled_content/Configuration/TypoScript/Static/<% if(t3x_tw_googleanalytics) { %>,EXT:tw_googleanalytics/Configuration/TypoScript<% } %><% if(t3x_squeezr) { %>,EXT:squeezr/Configuration/TypoScript<% } %>', 'compressJs = 0\r\ncompressCss = 0\r\ncompressHTML = 0\r\concatenateJs = 0\r\nconcatenateCss = 0\r\nconcatenateJsAndCss = 0', '<INCLUDE_TYPOSCRIPT: source="FILE:fileadmin/<%= project %>/.source/ts/10_main.t3s">');
